...
- [ ] city2graph セクションの縦積み表示を確認・調整
  - proximity + morphology: 1 列縦積み（sm 以下）
  - 地図高さ: モバイルで 300px（デスクトップ 400px）
- [ ] 凡例の折りたたみ実装
...
- [ ] 「値が 0」と「未取得」の表示差を目視確認
  - proximity: 施設 0 件 vs データ未取得
  - morphology: 密度 0 vs データ未取得
...
- [ ] proximity バーに `aria-label` 付与（例: 「食料品店: 最寄り 280m、3件」）
- [ ] morphology バーに `aria-label` 付与（例: 「建物密度: 3,200棟/km²（基準値の64%）」）
- [ ] 地図に `aria-label="分析地点と周辺施設の地図"` 付与
- [ ] 欠損表示に `role="status"` 付与
- [ ] 色だけに依存しない表現の確認
...
- [ ] `pnpm build` — ビルド成功
- [ ] `pnpm lint` — lint エラーなし
- [ ] モバイル表示（375px 幅）で全セクション確認
- [ ] フォールバック 8 パターン全て目視確認済み
- [ ] アクセシビリティ属性が全コンポーネントに付与済み

---

## 最終受け入れ確認

- [ ] city2graph 関連 UI がレポート前半の主要要素として表示される
- [ ] proximity でカテゴリ別の生活圏の差が一目で比較できる
- [ ] morphology で総合点だけでなく内訳 3 指標の意味が視覚的に理解できる
- [ ] 地図上で分析地点、徒歩圏、POI の関係が把握できる
- [ ] city2graph データが部分欠損しても UI 崩れや誤解を招く表示がない
- [ ] デスクトップとモバイルで表示が成立する
- [ ] `pnpm build` + `pnpm lint` + `pnpm test` が全て成功する
